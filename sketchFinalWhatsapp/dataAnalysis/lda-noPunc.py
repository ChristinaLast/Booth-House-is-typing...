...
with open("tmp031417/lda-noPunc"+str(numTopics)+"topics100.csv", "wb") as f:
    writer = csv.writer(f)
    for i in xrange(numTopics):
        writer.writerow([i, ldamodel.print_topic(i)])

print("topics", time.strftime("%H:%M:%S"))

ldamodel.save("tmp031417/lda-noPunc"+str(numTopics)+"topics100")
...
with open("tmp031417/lda-noPunc"+str(numTopics)+"topicBook100.csv", "wb") as f:
    ...
    for j in xrange(len(lookup)):
        ...
        temp = [j, lookup[str(j)]] + topics_flat + [doc_set[j]]
